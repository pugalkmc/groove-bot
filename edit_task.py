...
async def admin_mode(update, context):
    ...
    verify_id = db.reference(f"task_ids/{text}").get() or {}
    if verify_id is None:
        await bot.send_message(chat_id=chat_id, text="Task ID incorrect")
        return ADMIN_MODE
    task_details = db.reference(f"tasks/{verify_id['group_id']}").get() or {}
...
async def date_range(update, context):
    ...
    dict_payment = dict()
    get_peoples = db.reference('peoples').get() or {}
    while current_date <= end_date:
        ...
        dict_each_date = dict()
        # try:
        for i in each_date.values():
            user = i['username']
            if user not in dict_payment:
                get_user = db.reference(f"peoples/{i['chat_id']}").get() or {}
                if len(get_user) > 0 and "binance" in get_user:
        ...
        # # except:
        #     await bot.send_message(chat_id=chat_id, text=f"Error occured for date: {current_date.strftime('%d-%m-%Y')}")
        #     current_date += timedelta(days=1)
        #     continue
